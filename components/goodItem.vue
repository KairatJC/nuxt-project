<template>
    <div class="good-item">
        ...
        <p class="good-item__info">
            ...
            <span  class="good_price" :class="[isLow ? 'good__price--red' : 'good__price--green']">Цена: {{(dat.C * currency).toFixed(2)}}р.</span>
            <span>Количество: {{count}}шт.</span>
        </p>
        <button :disabled="!canAdd" @click="add_to_cart()">Добавить в корзину</button>
    </div>
</template>
<script>
...
export default {
    ...
    beforeMount(){
        ...
        // нужен для из-за того что при рендере, 
        // этот компонент может сначала отрендерится
        // не получить данные и выкинуть ошибку
        // если будут предложения как это лечить, с радостью узнаю
        if (this.names[this.dat.G].B[this.dat.T].N) {
    ...
    watch: {
        ...
        goods: {
            deep: true,
            handler() {
                this.count = this.goods[this.dat.index_of_goods].P - this.goods[this.dat.index_of_goods].counter
                if (this.count > 0) {
                    this.canAdd = true
                } else {
                    this.canAdd = false
